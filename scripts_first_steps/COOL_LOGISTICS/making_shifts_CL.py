...
def api_call_get_shiftplans():
    ...
    url = (f"https://shyftplan.com/api/v1/shiftplans?user_email={user_email}"
    f"&authentication_token={authentication_token}"
    f"&created_after={created_after}&order_key=id&order_dir=desc")
    headers = {"accept": "application/json"}
    response = requests.get(url, headers=headers)
    data = response.json()
    return data.get("items", [])
...
def api_call_post_shift(shiftplan_id, starts_at, ends_at, workers,
                        locations_position_id, note):
    payload = {
        "can_evaluate": True,
        "untimed": False,
...
def main():
    ...
    df_shifts = pd.read_csv("shifts_cl.csv")
    ...
    # Iterujemy przez każdą zmianę, definiujemy zmienne,
    # i pomijamy wszystko gdzie workers = 0
    ...
    for index, row in df_shifts.iterrows():
        ...
        # ---> 2023-06-18T19:00:00+02:00 w formacie ISO itd.
        starts_at_iso = start_datetime.isoformat()
        ends_at_iso = end_datetime.isoformat()

        # Zmieniamy z 2023-06-18T19:00:00+02:00
        # ---> 2023-06-18T19:00+02:00 (usuwamy sekundy) itd.
        ...
        locations_position_id = position_info["locations_position_id"
                                              ].values[0]
        shiftplan_id = position_info["shiftplan_id"].values[0]

        # Ustawienie kolejności.
        # Ustawiamy zmienna vehicle na jedna z 3 elementow tej tablicy -
        # to da nam gwarancje kolejnosci wrzucania zmian z priorytetem
        # poniewaz zmienna vehicle bedzie potem uzywana w kazdej
        # operacji dotyczacej co wrzucamy i dlaczego wrzucamy :)
        ...
        # break w przypadku gdy juz nie mamy co przypisywać,
        # przerywamy pętle dla tej iteracji
        if workers == 0:
            break
